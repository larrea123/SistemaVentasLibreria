...
class Reporte extends CI_Controller {
    ...
    {
        $lista = $this->reporte_model->reporteProducto1();
    ...
    public function productoFinal()
    {
        ...
        $producto= $this->reporte_model->reporteProducto($cantidad);
    ...
    public function reporteProductosPdf()
	{
        ...
        $this->pdf->SetFont('Arial', 'B', 11);
        $this->pdf->Cell(10, 8, 'No.', 'LTRB', 0, 'C', 1);
        $this->pdf->Cell(86, 8, utf8_decode('Detalle de Producto'), 1, 0, 'C', 1);
        $this->pdf->Cell(30, 8, utf8_decode('P/U (Bs.)'), 1, 0, 'C', 1);
        $this->pdf->Cell(16, 8, utf8_decode('Cant.'), 1, 0, 'C', 1);
        $this->pdf->Cell(33, 8, utf8_decode('Total (Bs.)'), 1, 1, 'C', 1);

        $lista=$this->reporte_model->reporteProducto($cantidad);
        $lista=$lista->result();
        $num = 1;
        $suma= 0;
        foreach($lista as $row){
            ...
            $this->pdf->SetFont('Arial', '', 8);
            $this->pdf->Cell(10, 5, $num, 1, 0, 'C', 0);
            $this->pdf->Cell(86, 5, utf8_decode($descripcion), 1, 0, 'L', false);
            $this->pdf->Cell(30, 5, utf8_decode($precio1), 1, 0, 'C', false);
            $this->pdf->Cell(16, 5, utf8_decode($cantidad), 1, 0, 'C', false);
            $this->pdf->Cell(33, 5, utf8_decode($total), 1, 0, 'C', false);
            $this->pdf->Ln();
            $num++;
        }   
